#!/usr/bin/env zsh
# gm_fix_zones_and_quicktest.zsh — create bridge zone dirs for all agents + drop a smoke-test WO
# Usage: zsh tools/ops/gm_fix_zones_and_quicktest.zsh
# Safe to re-run (idempotent mkdir + gitkeep).
set -euo pipefail

print_h(){ echo; echo "=== $1 ==="; }

# Resolve SOT (trusts ~/.luka_home if set)
...
# Drop a smoke-test WO into CLC inbox
...
# Optionally run router if present
