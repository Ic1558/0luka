...
def create_app():
    if FASTAPI_AVAILABLE:
        ...
        # AG-20: adaptation read-only endpoints
        try:
            from interface.operator.api_adaptations import (
                adaptations_list, adaptations_latest,
            )
            app.add_api_route("/api/adaptations", adaptations_list, methods=["GET"])
        ...
        # AG-34: supervised drift repair execution endpoints
        try:
            from interface.operator.api_drift_repair_execution import register_drift_repair_execution_routes
            register_drift_repair_execution_routes(app)
        except Exception:
            pass  # api_drift_repair_execution unavailable — skip gracefully
        # AG-35: repair verification + governance reconciliation endpoints
        try:
            from interface.operator.api_repair_reconciliation import register_repair_reconciliation_routes
        ...
        # AG-39: supervised repair wave scheduler endpoints
        try:
            from interface.operator.api_repair_wave import register_repair_wave_routes
