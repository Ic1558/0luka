...
def test_runtime_boundary_audit_clean_runtime_evidence_yields_zero_findings() -> None:
    with tempfile.TemporaryDirectory() as td:
        ...
        clean = root / "runtime" / "clean.py"
        clean_content = "DEFAULT_NAME = 'runtime_service'\\n"
        _write(clean, clean_content)
        old = _set_root(root)
        try:
            plan = {
                "id": "rba-domain-2",
                "trace_id": "trace-rba-domain-2",
                "author": "tester",
                ...
                "steps": [
                    ...
                    {"tool": "read_file", "params": {"path": "runtime/clean.py"}},
                ],
                "execution_contract": {
            ...
            after = clean.read_text(encoding="utf-8")

            assert before == clean_content
            assert after == clean_content
            assert violations == []
        finally:
            _restore_root(old)
...
if __name__ == "__main__":
    ...
    print("test_phase15_runtime_boundary_audit_clean_domain: all ok")
