...
class UniversalDistiller:
    ...
    def _smart_distill(self, nlp, mode):
        ...
        perfect = f"{arch['prefix']} Intent: {nlp}. {arch['logic']} Aesthetic: {arch['quality']} Camera: 24mm Wide Angle, Eye-level (1.6m)."
        
        return perfect

    def distill(self):
        logging.info(f"🚀 Initializing VISION-ENHANCED {self.mode.upper()} Engine...")
        ...
        perfect_prompt_content = self._smart_distill(self.user_nlp, self.mode)
        
        payload = {
            "trace_id": self.trace_id,
            "mode": self.mode.value,
            "fidelity_goal": 0.85,
            "vision_context": f"Analyzed {self.input_path.name} as {self.mode.value} source.",
            "perfect_prompt": perfect_prompt_content,
            "metadata": {
                "timestamp": datetime.now().isoformat(),
                "engine": "0luka_distiller_v2_vision_enhanced",
...
def process(
    ...
    distiller = UniversalDistiller(input_file, nlp, mode)
    distiller.distill()
