...
class GateRunnerDaemon:
    ...
    ACTION_TABLE = {
        ...
        "action.plan": "system/bin/liam-planner",
        "action.verify": "core_brain/ops/governance/codex_verifier.py"
    }
    ...
    def __init__(self):
        ...
        # v0.5.0 Startup Seals (The Golden State)
    ...
    def execute_action(self, task_id, action_id, args=None):
        ...
        try:
            env = os.environ.copy()
            if args:
                env["LUKA_ARGS_JSON"] = json.dumps(args)
                
            output = subprocess.check_output([str(handler_path)], stderr=subprocess.STDOUT, env=env).decode()
            tk_key = self.counter.get_next_tk()
    ...
    def _authorized_mutation(self, req):
        ...
        # 0.5.0 Allowlist: RPC Client (CLI) + Bridge Dispatcher (Automation)
        allowed_clients = {
            self.CLIENT_ID: self._rpc_client_path,
            "bridge_dispatcher": str(self.ROOT / "core_brain/ops/governance/bridge_dispatcher.py")
        }
        
        expected_path = allowed_clients.get(client_id)
        if not expected_path or client_path != expected_path:
            self._log_throttled(
                "unauthorized_mutation",
                f"SECURITY: Unauthorized mutation path client_id={client_id} client_path={client_path}. Expected={expected_path}"
            )
            return False
        return True
        ...
        try:
            ...
            try:
                if cmd in self.MUTATION_CMDS and not self._authorized_mutation(req):
                    ...
                    resp = self.execute_action(req["task_id"], req["action_id"], req.get("args"))
    ...
    def run(self):
        ...
        print(f"Judicial Daemon v0.5.0 listening on {self.SOCK_PATH}...")
